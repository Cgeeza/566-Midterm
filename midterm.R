...
print(paste(round(avg_standing_asymmetry*100, 4), "%", sep=""))
...
# Find unweighting phase: acceleration negative
...
print(paste(round(avg_unweighting_asymmetry*100, 4), "%", sep=""))
...
breaking_inds <- 974:1374
avg_breaking_asymmetry <- mean(data[breaking_inds, "abs_pct_diff"])
print(paste(round(avg_breaking_asymmetry*100, 4), "%", sep=""))

print(summary(data[breaking_inds, "abs_pct_diff"]))
print(sd(data[breaking_inds, "abs_pct_diff"]))
...
print(paste(round(avg_propulsion_asymmetry*100, 4), "%", sep=""))
...
df_plot <- data[c(standing_inds, unweighting_inds, breaking_inds,
                  propulsion_inds), ]
...
df_tidy_plot <- data_tidy[c(standing_inds, unweighting_inds, breaking_inds,
                            propulsion_inds, standing_inds + T,
                            unweighting_inds + T, breaking_inds + T,
                            propulsion_inds + T), ]
